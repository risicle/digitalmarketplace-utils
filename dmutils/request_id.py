...
class RequestIdRequestMixin(object):
    ...
    def get_onwards_request_headers(self):
        return dict(chain(
            ((header_name, self.trace_id) for header_name in current_app.config['DM_TRACE_ID_HEADERS'] if self.trace_id),
            ((header_name, self.span_id) for header_name in current_app.config['DM_SPAN_ID_HEADERS'] if self.span_id),
        ))
